...
--------------------------------------------------------------------------
- Commit inventory.xlsx to the repo. To update inventory, edit the file,
  commit, and push — Render redeploys automatically.
- "Send via Email" on Render uses SMTP (works anywhere). Set these
  environment variables in the Render dashboard:
      SMTP_HOST  e.g. smtp.gmail.com  (or smtp.office365.com)
      SMTP_PORT  587
      SMTP_USER  the sending address, e.g. you@gmail.com
      SMTP_PASS  an APP PASSWORD (not your normal password — see README)
      SMTP_FROM  optional; defaults to SMTP_USER
- Optionally set APP_PASSWORD to require a login before anyone can use
  the app. Strongly recommended once it's on the public internet.
- On your laptop (Windows + Outlook), the old Outlook automation still
  works with no SMTP setup — the app picks whichever is available.
- order_history.json lives on the server's disk. On Render's free tier
  the disk is EPHEMERAL: every deploy/restart wipes it, which resets the
  PO counter and history. See README for options.
"""
...
# SMTP settings, read from environment variables so no password ever gets
# committed to GitHub. If these are set, "Send via Email" uses SMTP; if
# not, it falls back to Outlook automation when running locally on Windows.
SMTP_HOST = os.environ.get("SMTP_HOST", "").strip()
SMTP_PORT = int(os.environ.get("SMTP_PORT", "587") or "587")
SMTP_USER = os.environ.get("SMTP_USER", "").strip()
SMTP_PASS = os.environ.get("SMTP_PASS", "").strip()
SMTP_FROM = os.environ.get("SMTP_FROM", "").strip() or SMTP_USER
SMTP_AVAILABLE = bool(SMTP_HOST and SMTP_USER and SMTP_PASS)
...
def email_status():
    """Tell the frontend whether 'Send via Email' can work, and how."""
    if SMTP_AVAILABLE:
        method = "smtp"
        ...
        method = None
    return jsonify({"available": method is not None, "method": method})
...
def export_email():
    if not (SMTP_AVAILABLE or OUTLOOK_AVAILABLE):
        return jsonify({
            "error": "Email isn't configured. On a server, set the SMTP_* "
                     "environment variables; on Windows, install Outlook + "
                     "pywin32. Use 'Export to Excel' to download instead."
        }), 400
